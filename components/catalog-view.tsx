...
export function CatalogView({ threads, boardCode }: CatalogViewProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-x-3 gap-y-6">
      {threads.map((thread) => (
        <Link
          key={thread.id}
        ...
        >
          ...
          <div className="relative w-full aspect-square bg-muted/20 border border-muted/40 rounded shadow-sm overflow-hidden transition-all duration-200 group-hover:scale-[1.02] group-hover:border-accent group-hover:shadow-md">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-2 pt-6 translate-y-1 group-hover:translate-y-0 transition-transform">
              <div className="flex items-center justify-between text-white font-mono text-[11px] font-bold">
                <span className="flex items-center gap-1 drop-shadow-sm">
                  R: {thread.replyCount}
                </span>
                <span className="flex items-center gap-1 opacity-90 text-[9px] font-light">
          ...
          {/* Metadata / Text */}
          <div className="mt-2 w-full text-center space-y-0.5 px-1">
            {thread.subject && (
              <h3 className="text-[13px] font-bold line-clamp-2 leading-[1.2] text-accent group-hover:underline decoration-1 underline-offset-2">
                {thread.subject}
              </h3>
            )}
            <p className="text-[11px] text-muted-foreground line-clamp-3 leading-tight font-medium opacity-80 group-hover:opacity-100 transition-opacity">
              {thread.content}
            </p>
          </div>
        </Link>
